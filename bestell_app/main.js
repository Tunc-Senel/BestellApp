...
// Active button image sources for nav bar buttons
 const activeButtonSources = [
    "./assets/img/favicons/home_active_nav.png",
...
// Closes the open dialog (desktop or mobile).
function closeDialog() {
  DIALOG_DESKTOP.close();
  DIALOG_MOBILE.close();
}

// Focuses on the open dialog(desktop or mobile) for tab navigation.
function focusDialog() {
  DIALOG_DESKTOP.focus();
  DIALOG_MOBILE.focus();
}

// Ensures the open dialog (desktop or mobile) is closed and hidden.
function hideVisibilityOfDialog() {
...
// Hides basket- and button-sectin of desktop and mobile setup.
function hideVisibilityofBasket() {
...
// adds backdrop and no-scroll to body
function addBackdropAndNoScroll() {
  BACKDROP.classList.add("active");
  BODY.classList.add("no-scroll");
}

// removes backdrop and no-scroll from body
function removeBackdropAndNoScroll() {  
...
function init() {
  ...
  BUY_NOW_BUTTON.addEventListener("click", () =>{
    ...
    openDialogOnBuyNow();
    focusDialog();
    hideVisibilityofBasket();
    addBackdropAndNoScroll();
    setTimeout(() => {
      closeDialog();
      hideVisibilityOfDialog();
      removeBackdropAndNoScroll()
    }, 3000);
  });
}
